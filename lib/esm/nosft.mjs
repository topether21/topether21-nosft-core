...
async function getInscriptionHtml(inscriptionId) {
  const response = await axios2.get(`${ordinalsExplorerUrl}/inscription/${inscriptionId}`);
...
async function fetchBitcoinPrice() {
  const bitcoinPriceResponse = await axios2.get(bitcoinPriceApiUrl);
  const bitcoinPriceData = bitcoinPriceResponse.data;
  const bitcoinPrice = bitcoinPriceData.USD.last;
  const recommendedFeeRateResponse = await axios2.get(`${baseMempoolApiUrl}/v1/fees/recommended`);
...
${utxo}`;
  ...
  async parseOrderEvent(order, orders = []) {
    ...
    if (isProcessed(orders, inscriptionId))
      return;
    const inscriptionDataResponse = await axios2.get(
